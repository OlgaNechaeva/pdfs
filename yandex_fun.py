...
def yandex_crawler(_search_engine, _file_type, list_of_keywords, _all_links_xpath, _link_xpath, _result_stat_xpath,
    ...
    for a_keyword in list_of_keywords:
        elem = _browser.find_element_by_name("text")
        elem.clear()
        elem.send_keys("{} f:{}".format(a_keyword, _file_type))  # in search box of the site
        sleep(5)
            ...
            try:
                ...
                if page == _pages_limit:
                    ...
                    for a_link in range(0, len(links)):
                        ...
                        if '.pdf' in ''.join(links[a_link].xpath(_link_xpath)):
                            link_count += 1
                            if link_count == _links_limit:
                                ...
                                if ''.join(links[a_link].xpath(_link_xpath)) not in all_links:
                                    ...
                                    dict_links.append({'link': ''.join(links[a_link].xpath(_link_xpath)),
                                                       'keyword': a_keyword,
                                                       'page_number': page,
                                                       'result_stats': ''.join(tree.xpath(_result_stat_xpath)).replace(
                                                           u'\xa0', ' '),
